            ...
            <div class="row justify-content-center text-center g-5">
                ...
                <div class="col-md-6 m-5">
                    ...
                    <ul class="nav nav-tabs justify-content-center border-0 mt-3 mb-3">
                        ...
                        <li class="nav-item">
                            <a class="nav-link text-dark custom-tab-active" href="">Vind-ik-leuks</a>
                        </li>
                        ...
                        <li class="nav-item">
                            ...
                            @else
                            <a class="nav-link text-dark custom-tab-nonactive" href="{{ route('profile.friends', [Auth::user()]) }}">Vrienden <span class="badge text-bg-secondary"></span></a>
                            @endif
                        </li>
                        <li class="nav-item">
                    ...
                    <div class="col text-start text-muted fs-6">
                        Aantal gelikete posts <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dot" viewBox="0 0 16 16">
                            <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
                        </svg> {{count($user->likes)}}
                    </div>
                        ...
                        <div class="card-body">
                            <div class="row mb-0">
                                <div class="col-md-10">
                                    ...
                                    <a href="{{ route('profile.profile', $post->post->user->name) }}" class="text-decoration-none"><img src="{{ strlen(explode('https', Auth::user()->image)[0]) == 0 && Auth::user()->provider_name != NULL ? Auth::user()->image : asset('uploads/profileImage/' . Auth::user()->image) }}" alt="avatar" width="32" height="32" class="rounded-circle me-2">{{ $post->user->name }}</a><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dot" viewBox="0 0 16 16">
                                        <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
                                    </svg>{{ $post->post->created_at->diffForHumans() }}<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dot" viewBox="0 0 16 16">
                                        <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
                                    </svg> {{$post->post->category->name}}
                                    @else
                                    <a href="{{ route('profile.user', $post->post->user->name) }}" class="text-decoration-none"><img src="{{ strlen(explode('https', $post->user->image)[0]) == 0 && $post->user->provider_name != NULL ? $post->user->image : asset('uploads/profileImage/' . $post->user->image) }}" alt="avatar" width="32" height="32" class="rounded-circle me-2">{{ $post->user->name }}</a><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dot" viewBox="0 0 16 16">
                                        <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
                                    </svg>{{ $post->post->created_at->diffForHumans() }}<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dot" viewBox="0 0 16 16">
                                        <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
                                    </svg> {{$post->post->category->name}}
                            ...
                            <div class="row text-center">
                                <div class="col">
                                    @if(count($post->post->comments) < 2) <a href="#" class="text-decoration-none" data-bs-toggle="modal" data-bs-target="#comment{{ $post->id }}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat" viewBox="0 0 16 16">
                                            <path d="M2.678 11.894a1 1 0 0 1 .287.801 10.97 10.97 0 0 1-.398 2c1.395-.323 2.247-.697 2.634-.893a1 1 0 0 1 .71-.074A8.06 8.06 0 0 0 8 14c3.996 0 7-2.807 7-6 0-3.192-3.004-6-7-6S1 4.808 1 8c0 1.468.617 2.83 1.678 3.894zm-.493 3.905a21.682 21.682 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a9.68 9.68 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7-3.582 7-8 7a9.06 9.06 0 0 1-2.347-.306c-.52.263-1.639.742-3.468 1.105z" />
                                        </svg> {{count($post->post->comments)}}</a>
                                        @else
                                        <a href="#" class="text-decoration-none" data-bs-toggle="modal" data-bs-target="#comment{{ $post->id }}">{{count($post->post->comments)}} Comments</a>
                                        @endif
                                </div>
                                <div class="col">
                                    @php
                                    ...
                                    @if($like)
                                    <form action="{{ route('posts.dislikePost',[$post->post->id, $like])}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    @if($post->likes < 2) <form action="{{ route('posts.LikePostStore',$post->post->id)}}" method="POST">
                                ...
                                <div class="modal-body text-start">
                                    ...
                                    <div class="row mb-0">
                                        <div class="col">
                                            ...
                                            <a href="{{ route('profile.user', $post->post->user->name) }}" class="text-decoration-none"><img src="{{ asset('uploads/profileImage/' . $post->post->user->image) }}" alt="avatar" width="32" height="32" class="rounded-circle me-2">{{ $post->post->user->name }}</a><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dot" viewBox="0 0 16 16">
                                                <path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z" />
                    ...
                    <div class="alert alert-warning text-center" role="alert">
                        Je hebt nog geen gelikte posts
                    </div>
